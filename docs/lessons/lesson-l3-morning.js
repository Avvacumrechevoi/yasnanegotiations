...
window.YasnaLessons.lessons.push(
  {
    ...
    blocks:[
      ...
      {type:'hero',
        ...
        title:'Утро начинается раньше будильника',
        lead:'На прошлом витке ты увидел, что у ночи есть ядро и две рамки. Сегодня та же логика раскроется в утре: то, что было частями одной полочки, развернётся в три отдельные.'},
      {type:'spiral-map',
        ...
        note:'**Ключевое продолжение**: в уроке 2 ты видел ядро и рамки ВНУТРИ одной полочки. В этом уроке та же тройная структура разворачивается на ТРЁХ полочках сразу. Так работает концентрическая спираль: одна идея, новый масштаб.'},
      {type:'toc',
        title:'Что будет в этом уроке',
        intro:'Разберём полочки 1, 2 и 3 — утреннюю фазу суток. Она устроена по той же логике, что и ночь, — убедишься сам.',
        ...
        items:[
          ...
          'Познакомимся с тремя полочками — **Искра, Заря, Утро** — и найдём в них знакомое «ядро и рамки».',
      ...
      {type:'three-shelves',
        ...
        items:[
          ...
          {id:'i4',label:'Открыл глаза. Лежишь, смотришь в потолок. Пока ничего не делаешь.',correct:2,
            explain:'Это Заря — физическое пробуждение без активности. Если, едва открыв глаза, ты хватаешь телефон, этой фазы у тебя просто нет.'},
      ...
      {type:'checkbox-quiz',
        ...
        items:[
          ...
          {id:'s1-partial',label:'Частично. Работают полочки 1 и 3, а Зари (полочки 2) нет',correct:true,
            feedbackOn:'Точно. «Туман» первые два часа — расплата за пропущенную Зарю. Мозг не успел переключиться из ночного режима: ему нужно 20–40 минут без активности. Решение простое — поставить будильник на 20 минут раньше и провести их без телефона и задач.',
            feedbackOff:''}
        ]},
      {type:'scenario',
        variant:'Случай 2',
      ...
      {type:'checkbox-quiz',
        ...
        items:[
          {id:'s2-yes',label:'Да. Все три полочки работают — Искра до 6:00, Заря 6:00–6:30, Утро с 6:30',correct:true,
            feedbackOn:'Да, это собранное утро. 30 минут Зари — немного, но на переключение хватает. «Перестал залипать в решениях» — как раз потому, что мозг успевает настроиться.',
            feedbackOff:''},
      ...
      {type:'checkbox-quiz',
        ...
        items:[
          {id:'s3-full',label:'Да. Полноценные 8–9 часов сна и пауза перед подъёмом',correct:false,
            feedbackOn:'Спит Мила действительно долго, но паузу перед подъёмом у неё съедает телефон. А это не Заря: телефон грузит мозг, тогда как Заре нужен покой, а не поток информации.',
            feedbackOff:''},
          {id:'s3-none',label:'Не совсем. Есть только полочка 3 — Искра и Заря сломаны',correct:true,
            feedbackOn:'Так и есть. Искра сломана: к 10:30 кортизол уже должен был выйти на пик, а Мила всё ещё спит. Зари нет — вместо покоя телефон. Осталось только Утро, и оно заходит с трудом. Вот откуда тексты, которые не даются.',
            feedbackOff:''}
        ]},
      ...
      {type:'final-quiz-inline',
        ...
        questions:[
          ...
          {q:'К какой полочке относится момент «открыл глаза, лежишь, пока ничего не делаешь»?',
           ...
           correctIdx:1,
           explain:'Полочка 2 — Заря. Физическое пробуждение без активности. Эта пауза нужна мозгу, чтобы переключиться из ночного режима в дневной. На неё уходит 20–40 минут — и у большинства городских жителей её просто нет.'},
          {q:'Что произойдёт, если утром пропустить полочку 2 (Зарю)?',
           ...
           correctIdx:1,
           explain:'Без Зари мозг не успевает переключиться: из сна — сразу в дела. Следствие — «туман» первые два часа, слабые решения, раздражительность. Классический случай — Светлана из нашего примера.'}
        ]},
      ...
      {type:'summary-block',
        ...
        points:[
          ...
          'Чаще всего выпадает Заря — без неё и появляется «туман» первые два часа дня.',
      ...
      {type:'spiral-map',
        ...
        upcomingLabels:'полочки 4, 5, 6 — путь к полудню',
        note:'**Что теперь работает:** ты видишь, что метод собирается из одного и того же принципа, который каждый раз разворачивается на новом уровне. Сутки разобраны — теперь разложи их сам, а дальше дерево ведёт к Двору: тот же круг, но на постройках и животных.'},
