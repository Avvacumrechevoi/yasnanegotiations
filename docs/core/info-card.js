...
/* Сноска-источник (страница книги, таймкод) с экрана снята: карточка
   говорит от продукта, а не «по книге». Поле s остаётся в данных. */
...
function собратьВопрос(n,i,p,cr,pr,суточная){
  ...
  /* Ложные варианты берём по КЛЮЧУ праны, а не по подписи: подписи в разных
     местах кода писались то «ШЕ», то «ШЕ», и правильный ответ попадал в список
     ложных вторым экземпляром — на местах 0, 4 и 8 человек видел «Земля ШЕ» и
...
function Info({i,p,af=[],y={},overlay=null,onEdit,onClose,onSel,onLesson,onTour,onYasna,откуда}){
  ...
  af.forEach(fid=>{
    ...
    if(fid==='arcs'){
      ...
      const arcRoles=[{r:'ФО',l:'исток',c:'#4090D8'},{r:'ЦИ',l:'нагрев',c:'#70B8F0'},{r:'ХА',l:'пик',c:'#F06838'},{r:'ШЕ',l:'остыв.',c:'#C0943A'},{r:'ФО',l:'конец',c:'#4090D8'}];
      arcs.forEach((arc,ai)=>{
        if(arc.includes(i)){
  ...
  /* Раньше здесь стояла цепочка else if, и на четырёх местах Суток из
     двенадцати разбор исчезал: урок перебивал его собой. Выходов из карточки
     столько, сколько их есть на самом деле. */
  const выходы=[];
  if(isEmpty&&onEdit) выходы.push({т:'Заполнить место '+i,на:()=>onEdit(i),главный:true});
  if(урок&&onLesson) выходы.push({т:'Урок · '+урок.t,на:()=>onLesson(урок.id)});
  if(естьТур&&onTour) выходы.push({т:'Разбор · '+(y.name||'эта ясна')+' по кругу',на:()=>onTour()});
  const действие=выходы[0]||null;
  const прочиеВыходы=выходы.slice(1);
  ...
  return(
    <div className="sht-korpus" ref={корень}>
      ...
      <div className="sht-telo">
        ...
        {прочиеВыходы.map((в,k)=><button key={k} type="button" className="dos-knopka dos-knopka--tihaya" onClick={в.на}>{в.т}</button>)}
        ...
        <figure className="dos-risunok">
          ...
          <figcaption>{суточная
            ?'Небо в это время. Восток слева, запад справа — как на чертеже.'
