...
(function () {
  ...
  function встроить(узел, опции) {
    ...
    /* ВЫХОД РИСУЕТСЯ СРАЗУ, ДО ОТВЕТА СЕРВЕРА.
       Раньше вся панель — поля, «Сохранить», «Выйти», «Удалить» — строилась
       внутри успешного ответа /account. Стоило запросу не пройти (метро,
       самолёт, упавший шлюз), и на экране оставалась одна строка «Не удалось
       загрузить данные аккаунта»: выйти из аккаунта было НЕЧЕМ. На общем
    ...
    выйтиРано.addEventListener('click', function () {
      ...
      Promise.resolve(logout()).then(function () {
        выйтиРано.disabled = false; выйтиРано.textContent = 'Выйти';
        if (о.приВыходе) о.приВыходе();
      });
    });
    подвал.appendChild(выйтиРано);
    узел.appendChild(подвал);
    ...
    api('/account').then(function (d) {
      ...
      try { console.warn('[аккаунт]', e); } catch (_) {}
      /* Сервер не ответил — поля показать нечем, но человек остаётся не
         запертым: «Выйти» стоит с самого начала, рядом ставим «Повторить».
         Сообщение говорит, что именно недоступно, а не «что-то пошло не так». */
      сост.appendChild(el('div', null, 'Личные данные сейчас не загрузить — нет связи с сервером. Вход, прогресс и выход работают.'));
      var снова = el('button', 'yac-btn', 'Повторить');
      снова.addEventListener('click', function () { встроить(узел, о); });
      подвал.insertBefore(снова, выйтиРано);
    });
  }
