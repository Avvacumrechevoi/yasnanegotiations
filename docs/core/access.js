/* ═══════════════════════════════════════════════════════════════════
   ...
   ПОРЯДОК РАЗРЕШЕНИЯ (повторяет серверный, но только для отрисовки):
     0. ключ cap:/role:/admin: — ПОЛНОМОЧИЕ, для него fail-open НЕ действует:
        открыто только суперадмину, по списку caps или явной строке сервера,
        всё остальное (нет данных, ключ неизвестен, ошибка) → ЗАКРЫТО
     ...
     3. персональный доступ (entitlements)     — длиннейший префикс,
        ...
        строки с cap:/role:/admin: игнорируются (как на сервере)
...
(function () {
  ...
  // ПОЛНОМОЧИЯ — исключение из fail-open. cap:/role:/admin: — это власть
  // (публикация, раздача прав, админка), а не контент: «прав не знаем» и
  // «ключ неизвестен» для них означают ЗАКРЫТО. Регэксп зеркалит серверный
  // RESERVED_RE из server/access.js — источники истины должны совпадать.
  var POWER_RE = /^(cap|role|admin)([:._*-]|$)/i;
  function isPower(f) { return POWER_RE.test(f); }
  ...
  var DEFAULT_HINTS = {
    wip: 'Этого раздела в приложении пока нет.',
  ...
  function normEnts(src) {
    ...
    for (i = 0; i < src.length; i++) {
      ...
      f = String(e.feature);
      // Полномочия через персональные строки не ходят: сервер такие
      // отбрасывает (RESERVED_RE в /access/grant и resolveAccess), и клиент
      // не должен позволять просроченному кэшу открыть cap:.
      if (isPower(f)) continue;
      out[f] = !(e.granted === false);
    }
    return out;
  }
  ...
  function resolve(feature, s) {
    ...
    // Прав не знаем — контент открыт (ни сеть, ни недоехавший деплой, ни
    // первый заход не имеют права закрыть раздел), но ПОЛНОМОЧИЯ закрыты:
    // кнопка «опубликовать» не должна появляться из-за отвалившегося pull.
    if (!s.hasData) {
      if (power) return verdict(f, false, 'denied', 'fail-closed', null, node);
      return verdict(f, true, 'no-data', 'fail-open', null, node);
    }
    ...
    // Полномочия: только список caps (его собрал сервер из галочек роли)
    // либо явная строка в готовом ответе. Всё остальное — закрыто.
    if (power) {
      var i, capMap = {};
      for (i = 0; i < s.caps.length; i++) capMap[s.caps[i]] = true;
      var cp = matchRule(capMap, f);
      if (cp && cp.granted) return verdict(f, true, 'granted', 'caps', cp.key, node);
      var srvP = matchRule(s.rules, f);
      if (srvP) return verdict(f, srvP.granted, srvP.granted ? 'granted' : 'denied', 'server', srvP.key, node);
      return verdict(f, false, 'denied', 'fail-closed', null, node);
    }
  ...
  // ─── публичные вопросы ──────────────────────────────────────────
  // Внутренняя ошибка по КОНТЕНТУ обязана давать true: сломанный резолвер
  // не имеет права закрыть человеку продукт. По ПОЛНОМОЧИЯМ — наоборот:
  // сломанный резолвер не имеет права показать кнопку публикации.
  function can(feature, altState) {
    try { return ask(feature, altState).allowed; }
    catch (e) {
      warn('can(' + feature + ') упал: ' + ((e && e.message) || e));
      return !isPower(String(feature || ''));
    }
  }
  function why(feature, altState) {
    try { return ask(feature, altState); }
      ...
      var p = isPower(String(feature || ''));
      return verdict(String(feature || ''), !p, p ? 'denied' : 'error', p ? 'fail-closed' : 'fail-open', null, null);
    }
  }
  function role() {
    var r = current().role;
