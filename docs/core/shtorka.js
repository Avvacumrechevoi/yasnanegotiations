...
function создать(корень, наст) {
  ...
  function пересчитать() {
    ...
    var мини   = Math.min(320, Math.max(188, Math.round(H * 0.235))) + док;
    ...
    поставить(Y[упор] != null ? Y[упор] : Y.мини, false);
  }
  var низкий = false;
  ...
  function тягаНачать(y) {
    ...
    перо = { y0: y, старт: целевой, упорНаСтарте: упор, точки: [{ t: Date.now(), y: y }] };
  }
  ...
  function кУпору(имя) {
    ...
    if (имя === 'закрыто' && наст.наЗакрытие) {
      setTimeout(function () { if (!снят && наст.наЗакрытие) наст.наЗакрытие(); }, длит + 20);
    }
  }
  ...
  function конец(e, x, y) {
    ...
    if (Math.abs(dx) < ТАП && Math.abs(dy) < ТАП && быстро) {
      if (начало.ручка) { упор === 'полно' ? шагВниз() : шагВверх(); }
