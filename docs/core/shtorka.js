...
function создать(корень, наст) {
  ...
  /* ── Роль и фокус ────────────────────────────────────────────────────
     Карточка выезжала поверх круга обычным aside: роли у неё не было, а
     фокус оставался на звезде — человек тапнул место и не слышал, что
     открылось, и искал карточку свайпами через весь экран.
     Диалог НЕмодальный (aria-modal и inert не ставим): круг под карточкой
     остаётся рабочим, с него выбирают соседнее место — гасить его нельзя.
     Имя у карточки своё («Карточка места», ставит info-card), поэтому
     помощник его не трогает; фокус ведём на имя места в шапке — вслух
     читается ровно то, что человек только что выбрал. */
  var снятьСлой = null;
  var помощник = window.YasnaOkna && window.YasnaOkna.слой;
  if (помощник) снятьСлой = помощник(корень, {
    модальный: false,
    фокус: корень.querySelector('.sht-shapka h2') || корень.querySelector('h2')
  });
  ...
  return {
    ...
    снять: function () {
      снят = true;
      if (снятьСлой) { снятьСлой(); снятьСлой = null; }
