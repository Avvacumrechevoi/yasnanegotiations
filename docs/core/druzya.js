/* ═══════════════════════════════════════════════════════════════════════════
   ДРУЗЬЯ — свои люди в Ясне: код, заявки, список, приглашение в Партию.

   ГДЕ ЖИВЁТ ПРАВДА. В таблицах базы (миграция 006, server/druzya.js):
     druzya_ludi   pid, user_id, device_id, nick, avatar, kod
     druzhba       (pid, drug_pid) → poslal | prishla | druzya
   Обе половины пары меняются одним запросом, поэтому разойтись не могут.

   ПОЧЕМУ ПЕРЕЕХАЛИ. Раньше дружба целиком лежала в дереве Firebase, а
   личностью человека была запись в памяти телефона. Из этого следовало всё
   сразу: переустановил приложение — стал другим человеком; «убрать друга»
   правило разрешало только в своей половине, и убранный продолжал видеть и
   звать; согласие ложилось в ящик otvety и дружба завершалась лишь тогда,
   когда позвавший сам откроет приложение; мусор от прежних личностей
   оставался навсегда (клиентская уборка в дереве невозможна); частота заявок
   не считалась вовсе. Ни одна из этих бед не чинилась правилами — все они
   следовали из того, ГДЕ лежат данные.

   ЧТО ОСТАЛОСЬ В ДЕРЕВЕ. Зовы в Партию: им нужна секунда доставки.
     lyudi/<pid>            { nick, avatar, uid, ts }   — карточка (ТЕНЬ)
     druzya/<чей>/<друг>    { nick, avatar, ts }        — список (ТЕНЬ)
     zovy/<кому>/<pushId>   { code, kind, ot{…}, ts }   — зов в комнату
   Карточка и список здесь — ТЕНЬ, а не правда: правило зовов требует, чтобы
   ПОЛУЧАТЕЛЬ имел зовущего в СВОЁМ узле druzya/, а писать туда может только
   он сам. Поэтому каждый отражает свой список в дерево. Когда владелец
   опубликует правило зовов без проверки дружбы, тень можно снять целиком.
   Узлы kody/, zayavki/ и otvety/ больше не используются вовсе.

   Владение своим узлом в дереве доказывается как у слотов комнаты: первый,
   кто застолбил lyudi/<pid>, пишет туда свой uid, и дальше писать может
   только он (TOFU — доверие первому).

   Код комнаты = право её читать, поэтому раздавать его кому попало нельзя.
   ═══════════════════════════════════════════════════════════════════════════ */
(function () {
  'use strict';
