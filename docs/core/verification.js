...
function Verification({y,vs,setVs,onClose}){
  ...
  const Check=({id,weight,q,info,hint})=>{const v=get(id);const dim=hidePassed&&v===true;
    ...
    <div data-check-id={id} style={{display:'flex',alignItems:'flex-start',gap:10,padding:'10px 0',borderBottom:'1px solid #f0f0f2'}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        {hint&&v===false&&<div style={{fontSize:11,color:'var(--txt2,#6e6e73)',marginTop:4,padding:'6px 10px',background:'var(--bg2,#f5f5f7)',borderRadius:5,borderLeft:'2px solid #E8A834',lineHeight:1.5}}>💡 {hint}</div>}
      </div>
    </div>);};
  ...
  const expressChecks=[
    ...
    {id:'ex_twelve',w:'КРИТ',
      q:'Ровно 12 различных мест — не больше и не меньше?',
    ...
    {id:'ex_closure',w:'КРИТ',
      q:'Цикл замыкается: место 11 переходит в место 0 = новое начало?',
    ...
    {id:'ex_param',w:'ВАЖ',
      q:'Один параметр для всех 12 мест (например, свет, тепло, возраст)?',
      info:'**Ясна Года №3:** «Закон устройства всех явлений природы.» Один параметр — то, что нарастает к месту 6 и убывает к месту 0.'},
    {id:'ex_unique',w:'ВАЖ',
  ...
  const krestChecks=[
    ...
    {id:'k_3_9_horizon',w:'КРИТ',
      q:`Места 3 («${lbl(3)}») и 9 («${lbl(9)}») лежат на Линии Горизонта — главные точки противоборства?`,
    ...
    {id:'k_0_6_unity',w:'КРИТ',
      q:`Места 0 («${lbl(0)}») и 6 («${lbl(6)}») — крайности параметра, антиподы по Линии Единения?`,
  ...
  const polkiChecks=[
    ...
    {id:'p_each_property',w:'КРИТ',
      q:'Каждое место имеет своё уникальное свойство?',
      info:'**Ясна Суток №93:** «Каждое время имеет своё собственное свойство.»'},
    {id:'p_each_event',w:'ВАЖ',
      q:'На каждом месте происходит конкретное событие?',
      info:'**Ясна Суток №94:** «В каждом из времён происходит некое событие.»'},
    {id:'p_long_smooth',w:'КРИТ',
      q:'Все 6 долгих мест (0,2,4,6,8,10) — времена плавных, медленных изменений обстановки?',
      ...
      hint:'Если на чётном месте мгновенное событие — оно не долгое. Переформулируйте.'},
    {id:'p_short_transition',w:'КРИТ',
      q:'Все 6 коротких мест (1,3,5,7,9,11) — переходы от одного долгого к следующему?',
      ...
      hint:'Если на нечётном месте длительный процесс — оно не короткое. Переформулируйте.'},
    ...
    {id:'p_closure',w:'КРИТ',
      q:'Место 11 → место 0 = переход в новый цикл (замыкание есть)?',
  ...
  const krestyAndPranChecks=[
    {id:'kp_right',w:'КРИТ',
      ...
      info:'**Ясна Суток №446–447, 502:** Крест Управления / Любви — следствия. 0→1, 3→4, 6→7, 9→10. На каждом таком месте — результат предыдущего опорного.'},
    {id:'kp_left',w:'КРИТ',
      q:`Крест Веры (2,5,8,11): «${lbl(2)}», «${lbl(5)}», «${lbl(8)}», «${lbl(11)}» — все подготовки к следующим опорам?`,
      info:'**Ясна Суток №452, 499:** Крест Веры / Левит — подготовка. 2→3, 5→6, 8→9, 11→0. Каждое такое место готовит следующее опорное.'},
    {id:'kp_triples',w:'ВАЖ',
    ...
    {id:'kp_arcs',w:'ЖЕЛ',
      q:'Три Дуги (1-2-3-4-5, 5-6-7-8-9, 9-10-11-0-1) — каждая описывает плавный полный поворот через 5 мест?',
  ...
  const wholeChecks=[
    ...
    {id:'w_mobius',w:'ВАЖ',
      q:'Замыкание (Лента Мёбиуса): место 11 готовит место 0 = 12 как один цикл?',
      info:'**Ясна Года узел 8 №569–570:** «Змей Питон, заглотивший хвост — древняя Лента Мёбиуса». **Ясна Жизни узел 12 №952:** место 0 = Зачатие нового цикла.'},
    {id:'w_accumulation',w:'ВАЖ',
      q:'Закон Накопление→Переход: длинные места копят, соседние короткие — точки перелива?',
    ...
    {id:'w_param_unique',w:'КРИТ',
      q:'Параметр действительно ОДИН и тот же на всех 12 местах, нигде не «соскакивает» на другую шкалу?',
      ...
      hint:'Если параметр «гуляет» от места к месту — Ясна построена по двум разным шкалам сразу.'},
    {id:'w_no_dup',w:'ВАЖ',
    ...
    {id:'w_coherent',w:'ВАЖ',
      q:'Прочитав 12 мест подряд — получаю цельный образ явления?',
      info:'Простая проверка на цельность. Если после 12 мест остаётся ощущение разрозненности — не всё стоит на своих местах.'},
    {id:'w_explain',w:'ЖЕЛ',
  ...
  const exportReport=()=>{
    ...
    [['Шаг 0. Быстрая проверка',expressChecks],['Шаг 1. Опорный Крест',krestChecks],['Шаг 2. 12 мест',polkiChecks],['Шаг 3. Кресты + Праны',krestyAndPranChecks],['Шаг 4. Целое',wholeChecks]].forEach(([name,checks])=>{
    ...
    return(
      <div style={{position:'fixed',top:0,left:0,width:'100%',height:'100%',background:'var(--bg,#fff)',color:'var(--txt,#1d1d1f)',zIndex:130,display:'flex',flexDirection:'column'}}>
        ...
        <div className='fullpage-content' style={{flex:1,overflowY:'auto',padding:'24px 20px',maxWidth:680,margin:'0 auto',width:'100%'}}>
          ...
          {hasEmpties&&<div style={{padding:'12px 14px',background:'var(--bg2,#f5f5f7)',border:'1px solid var(--border,#d2d2d7)',borderLeft:'3px solid #E8A834',borderRadius:10,marginBottom:20,fontSize:13,color:'var(--txt2,#6e6e73)',lineHeight:1.55}}>
            <div style={{fontWeight:700,color:'var(--txt,#1d1d1f)',marginBottom:4}}>⚠ Не заполнены: {emptyPos.join(', ')}</div>
            Сначала заполните все 12 мест в Редакторе.
          </div>}
          ...
          <div style={{display:'flex',flexDirection:'column',gap:10,marginBottom:24}}>
            <div style={{padding:'14px 16px',background:'rgba(0,122,255,.04)',borderRadius:12,borderLeft:'3px solid #0071e3'}}>
              ...
              <div style={{fontSize:13,color:'var(--txt2,#6e6e73)',lineHeight:1.55}}>Самое важное одной обоймой: Горизонт → Опорный Крест → 12 мест → чередование → замыкание → параметр → уникальность.</div>
            </div>
            ...
            <div style={{padding:'14px 16px',background:'var(--bg2,#f5f5f7)',borderRadius:12,borderLeft:'3px solid #6e6e73'}}>
              <div style={{display:'flex',alignItems:'baseline',gap:8,marginBottom:4}}>
                ...
                <div style={{fontSize:15,fontWeight:600,color:'var(--txt,#1d1d1f)'}}>12 мест ({polkiChecks.length})</div>
              </div>
              <div style={{fontSize:13,color:'var(--txt2,#6e6e73)',lineHeight:1.55}}>Главные свойства мест из Ясны Суток, узел 2, пп. 88–97: 12 различных, 6 долгих + 6 коротких, у каждого свойство и событие, цикл замыкается.</div>
            </div>
            <div style={{padding:'14px 16px',background:'var(--bg2,#f5f5f7)',borderRadius:12,borderLeft:'3px solid #6e6e73'}}>
              <div style={{display:'flex',alignItems:'baseline',gap:8,marginBottom:4}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:6,marginBottom:24}}>
            <div style={{display:'flex',alignItems:'center',gap:10,padding:'8px 12px',background:'#fff5f5',borderRadius:8,border:'1px solid #E8364F20'}}>
              ...
              <span style={{fontSize:13,color:'var(--txt2,#6e6e73)'}}>Без неё Ясна не считается верной. Цитата из книги всегда указана.</span>
            </div>
            <div style={{display:'flex',alignItems:'center',gap:10,padding:'8px 12px',background:'var(--bg2,#f5f5f7)',borderRadius:8,border:'1px solid #E8A83440'}}>
              ...
              <span style={{fontSize:13,color:'var(--txt2,#6e6e73)'}}>Влияет на итог: «безупречно / с оговорками».</span>
            </div>
  ...
  return(
    <div style={{position:'fixed',top:0,left:0,width:'100%',height:'100%',background:'var(--bg,#fff)',color:'var(--txt,#1d1d1f)',zIndex:130,display:'flex',flexDirection:'column'}}>
      ...
      <div style={{display:'flex',gap:4,padding:'8px 20px',borderBottom:'1px solid #f0f0f2',flexShrink:0,overflowX:'auto'}}>
        ...
        <TabBtn id="polki" label="Шаг 2 · 12 мест" s={polkiScore}/>
      ...
      <div className='fullpage-content' style={{flex:1,overflowY:'auto',padding:'16px 20px',maxWidth:820,margin:'0 auto',width:'100%'}}>
        {hasEmpties&&<div style={{padding:'10px 14px',background:'var(--bg2,#f5f5f7)',border:'1px solid var(--border,#d2d2d7)',borderLeft:'3px solid #E8A834',borderRadius:10,marginBottom:14,fontSize:12,color:'var(--txt2,#6e6e73)',lineHeight:1.55}}>
          <div style={{fontWeight:700,color:'var(--txt,#1d1d1f)',marginBottom:4}}>⚠ Не заполнены места: {emptyPos.join(', ')}</div>
          Проверка достоверна только на полной Ясне. Сначала заполните все 12 мест в Редакторе.
        </div>}
        ...
        {tab==='fast'&&<>
          <div style={{fontSize:12,color:'#1d4ed8',marginBottom:12,lineHeight:1.55,padding:'10px 12px',background:'rgba(0,122,255,.06)',borderRadius:8,borderLeft:'3px solid #0071e3'}}>
            <b>⚡ Шаг 0. Быстрая проверка</b> — 7 пунктов ядра. Прямой проход по канону: алгоритм Дианоме (Ясна Года №11–16) + свойства 12 мест (Ясна Суток №88–97) + замыкание (Ясна Суток №73). ~3 минуты.
          </div>
        ...
        {tab==='polki'&&<>
          <div style={{fontSize:12,color:'var(--txt2,#6e6e73)',marginBottom:12,lineHeight:1.55,padding:'9px 12px',background:'#f0f5ff',borderRadius:8}}>
            <b style={{color:'#0071e3'}}>Шаг 2. 12 мест</b> — ключевые определения из Ясны Суток, узел 2, пункты 88–97. Самая короткая формулировка во всех трёх книгах.
          </div>
          <div style={{border:'1px solid #e5e5ea',borderRadius:10,padding:'4px 14px'}}>
