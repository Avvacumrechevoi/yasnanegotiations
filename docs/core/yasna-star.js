...
function Star({yy,sel,onSel,hl,af=[],showOpp,overlay,mob,drill,onDrill,subPolki,starRotation,rotationSpeed,showComposition}){
  ...
  // ЦВЕТ ПОЛОЧКИ — ПО ТРЕУГОЛЬНИКУ СТИХИИ, а не по кресту.
  // Четыре треугольника дают ровно четыре цветовые группы по три полочки:
  //   Земля 0·4·8 · Вода 1·5·9 · Воздух 2·6·10 · Огонь 3·7·11
  // (i % 4 попадает в треугольник, потому что шаг треугольника по кругу равен 4).
  // Крест при этом не теряется: опорный по-прежнему выделяется весом обводки
  // (strokeWidth ниже), то есть два разных смысла несут два разных признака —
  // цвет говорит о стихии, толщина о кресте.
  const gp=i=>['she','fo','tsi','ha'][i%4];
  const pc=i=>PR[gp(i)].c;
  const nc=i=>{
    if(hl) return hl.includes(i)?pc(i):'#e0e0e8';
    // Выбор: красим ТОЛЬКО выбранный узел.
  ...
  /* Поле зрения на телефоне было шире рисунка на треть: круг занимал 44%
     ...
     Слева оставлено на 24 единицы больше: самая длинная подпись слева
     («Утренняя Заря / Рассвет») уходила за край экрана на 11 px. */
  return(
    <svg viewBox={mob?`2 -10 890 720`:`-80 -50 1060 800`} preserveAspectRatio="xMidYMid meet" style={{width:'100%',height:'100%',display:'block'}}>
      <defs>
      ...
      {!overlay&&(starRotation?lpsRot:lps).map((pt,i)=>{const lOrig=p[i]||'';if(!lOrig)return null;let dy=5;if(!starRotation){if(i===0)dy=16;if(i===6)dy=-7;}
        ...
        /* НА КРУГЕ — ОДНО ИМЯ МЕСТА. В данных у части мест записано два
           названия через косую черту («Запад / Вечер Сутки», «Утренняя Заря /
           Рассвет», «Закат / Вечерняя Заря»). На круге они занимали две
           строки и столько ширины, что рисунок нельзя было увеличить: подписи
           боковых мест 2, 8 и 9 упирались в край экрана. Второе имя никуда не
           пропало — оно живёт в карточке места, где его есть где прочитать
           вместе с объяснением. Так же сделано в игре «Разложи по кругу»:
           на кольце первое имя, варианты в карточке. */
      ...
      {overlay&&<>
        ...
        {Array.from({length:12},(_,i)=>{const op=xy(i,cx,cy,lr+12);return<circle key={`os${i}`} cx={op.x} cy={op.y} r="3" fill={sel===i?'#7c3aed':'#9333ea'} opacity={sel===i?.9:.5} style={{pointerEvents:'none'}}/>;})}
        {/* Thin connector lines between inner point and outer satellite */}
        {/* Спутники и связки наложения рисуются ПОСЛЕ узлов, поэтому без
            pointerEvents:none они молча съедали тап по местам 0, 6 и 9 —
