// ═══════════════════════════════════════════════════════════════════
// Запасной банк вопросов · Игра по Ясне
// 9 тем × 5 вопросов = 45 вопросов, мир «Сутки».
// Работает только когда не поднялся content.bundle.js (см. useNew ниже):
// это страховка, а не основной банк. Его вопросы собраны не по книгам
// напрямую — если он когда-нибудь всплывёт на людях, сверять с книгой.
// ═══════════════════════════════════════════════════════════════════
(function(){
  const THEMES = [
  ...
  const QUESTIONS = [
    ...
    { id:'sutki-1', theme:'sutki', diff:1,
      text:'Сколько мест в Ясне Суток?',
      options:['7','10','12','24'],
      correct:2,
      hint:'12 мест = 24 часа / 2. На одном месте — два часа суточного цикла.' },
    { id:'sutki-2', theme:'sutki', diff:1,
      text:'Какое место отвечает полночи?',
      options:['0 (она же 12)','6','3','9'],
      correct:0,
      hint:'Полночь — точка полного покоя, начало нового цикла, место 0/12.' },
    { id:'sutki-3', theme:'sutki', diff:1,
      text:'Какое место отвечает полудню?',
      options:['0','6','12','18'],
      correct:1,
      hint:'Полдень — пик светового дня, противоположное полночи место 6.' },
    { id:'sutki-4', theme:'sutki', diff:1,
      text:'Сколько часов на одном месте Суток?',
      options:['1','2','3','4'],
      correct:1,
      hint:'24 часа делятся на 12 мест → по 2 часа на каждое.' },
    ...
    { id:'zerno-3', theme:'zerno', diff:3,
      ...
      options:['На крайних местах','В центре Чертежа','В корне Звезды','На антиподах'],
      correct:1,
    ...
    { id:'antipody-1', theme:'antipody', diff:1,
      text:'Антипод места 0 в Ясне Суток — это место…',
      options:['6','3','9','12'],
      correct:0,
      hint:'Антипод — место ровно напротив на круге, через шесть шагов.' },
    { id:'antipody-2', theme:'antipody', diff:1,
      text:'Что такое антипод?',
      options:['Враг','Дубликат','Противоположное место по оси','Соседнее место'],
      correct:2,
      hint:'Антипод — место, симметричное относительно центра Звезды.' },
    { id:'antipody-3', theme:'antipody', diff:2,
      text:'Сколько пар антиподов всего в Ясне Суток?',
      options:['3','4','6','12'],
      correct:2,
      hint:'12 мест ÷ 2 = 6 пар противоположностей.' },
    { id:'antipody-4', theme:'antipody', diff:1,
      text:'Антипод места 1 — это…',
      options:['5','11','7','13'],
      correct:2,
      hint:'1 + 6 = 7. Каждый антипод стоит ровно через шесть мест.' },
    ...
    { id:'skorpion-1', theme:'skorpion', diff:3,
      ...
      options:['Знак Зодиака на месте 7','Особая точка между местами, мост-связка','Имя главного героя','Время в году'],
      correct:1,
      hint:'Скорпион — особая механика, точка вне двенадцати мест.' },
    { id:'skorpion-2', theme:'skorpion', diff:3,
      ...
      correct:1,
      hint:'Скорпион и Паук — парные особые механики, они дополняют двенадцать мест.' },
    ...
    { id:'skorpion-4', theme:'skorpion', diff:3,
      ...
      options:['Это особые точки','Они нарушают порядок двенадцати мест','Они появляются в каждой Ясне (Сутки, Года, Жизни)','Они показаны вне круга'],
      correct:2,
      hint:'Скорпион/Паук — особенность именно Ясны Суток, не во всех Яснах одинаково.' },
    { id:'skorpion-5', theme:'skorpion', diff:3,
      text:'На какой паре мест «работает» Скорпион Суток?',
    ...
    { id:'chashi-1', theme:'chashi', diff:2,
      ...
      correct:1,
      hint:'Чаша наполняется силой по мере прохождения мест Суток.' },
    { id:'chashi-2', theme:'chashi', diff:2,
      text:'Когда Чаша Тьмы наполнена максимально?',
      options:['В полночь (место 0)','В полдень (место 6)','На рассвете (место 3)','Никогда полностью'],
      correct:0,
    ...
    { id:'chashi-4', theme:'chashi', diff:1,
      ...
      options:['Одна (общая)','Две (Света и Тьмы)','Четыре (по сторонам)','Двенадцать (по местам)'],
      correct:1,
    ...
    { id:'chashi-5', theme:'chashi', diff:3,
      ...
      options:['По мере перехода через места Суток','Случайно','По воле наблюдателя','Никогда не меняются'],
      correct:0,
      hint:'Каждое место отдаёт каплю в Чашу: Свет растёт от полночи к полудню, Тьма — наоборот.' },
    ...
    { id:'prana-5', theme:'prana', diff:3,
      ...
      options:['Каждой стихии — одно место','Каждой стихии — три места (тригон)','Стихии вне Чертежа','Стихии случайно распределены'],
      correct:1,
      hint:'Тригон Стихии — три места, они образуют равносторонний треугольник на круге.' },
    ...
    { id:'zerkalo-2', theme:'zerkalo', diff:3,
      ...
      correct:2,
      hint:'Через Зеркало Девы видна симметрия — у каждого места есть своё отражение.' },
    ...
    { id:'skrizhal-3', theme:'skrizhal', diff:3,
      ...
      options:['Можно идти только одной дорогой','Прана движется по трём основным потокам','Звезда имеет три луча','Три места — главные'],
      correct:1,
    ...
    { id:'skrizhal-5', theme:'skrizhal', diff:3,
      ...
      options:['Чертёж 144 мест','Только зодиак','Сутки Рыболова — итоговая интеграция','Описание ума'],
      correct:2,
  ...
  function rebuild(){
    ...
    /* Вопрос темы, которой нет в игре, — не вопрос банка. Раньше сюда падали
       все вопросы подряд, включая темы с themes_in_partia=false: партия их и
       так не брала (она перебирает ACTIVE_THEMES), но getAllQuestions() отдавал
       их наружу, и любое число «столько-то вопросов» получалось завышенным.
       Считаем банк по тому, во что действительно можно сыграть. */
    const темыИгры = new Set(ACTIVE_THEMES.map(t => t.id));
    ACTIVE_QUESTIONS = raw.filter(q => isQuestionValid(q) && темыИгры.has(q.theme));

    const skipped = raw.length - ACTIVE_QUESTIONS.length;
    if(skipped > 0){
      console.log('[YasnaTrivia] Отфильтровано', skipped,
        'вопросов (битые, отключённые типы или темы вне игры:',
        [...DISABLED_TYPES].join(', '), ')');
    }
  }

  /* ─── Сколько всего ────────────────────────────────────────────────────
     Единственное место, откуда берут число вопросов и тем для показа
     человеку. Экраны и дерево знаний раньше писали это число руками —
     в дереве стояло «160 вопросов» при банке в другую сотню, и число
     врало каждому, кто открывал игру. Пусть спрашивают у данных. */
  function счёт(){
    return { тем: ACTIVE_THEMES.length, вопросов: ACTIVE_QUESTIONS.length };
  }
  ...
  window.YasnaTrivia = {
    ...
    getAtoms, getQuestionsFull, счёт,
    MODE_CONFIG,
