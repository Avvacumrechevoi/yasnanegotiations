...
(function(){
  ...
  // Иерархия: «Вопрос N/M» (пилюля) → «Тема: …» (приглушённо) → текст вопроса.
  // Таймер-чип убран (есть полоса обратного отсчёта), тип вопроса не дублируем —
  // он и так очевиден из интерфейса вариантов (а для match-pair есть подсказка).
  function TnQuestionCard({ qOverall, totalOverall, themeName, text, timeLeft, showFeedback, qType }){
    // «Джива: Как работает…» → «Тема: Джива. Как работает…» (внутреннее двоеточие → точка)
  ...
  function TnMatchPair({ pairsLeft, pairsRight, correct, chosen, showFeedback, onSubmit }){
    ...
    return React.createElement('div', {
      ...
      React.createElement('div', { className: 'tn-match-grid' },
        pL.map((txt, i) => {
          // ── Левая ячейка (фиксированный порядок) ──
          ...
          const leftBtn = React.createElement('button', {
            key: 'l' + i, type: 'button',
          ...
          // ── Правая ячейка (перемешанная) — бейдж слева (смотрит в центр) ──
          ...
          const rightBtn = React.createElement('button', {
            key: 'r' + rc.origIdx, type: 'button',
  ...
  try {
    if (window.YasnaAccess && window.YasnaAccess.declare) {
      window.YasnaAccess.declare({ feature: 'game:turnir', area: 'game', parent: 'section:game',
