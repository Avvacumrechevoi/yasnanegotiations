/* ═══════════════════════════════════════════════════════════════════════════
   ...
   ПРАВИЛА, КОТОРЫЕ ЗДЕСЬ ЖИВУТ (условия выкатки из отчёта §5):
     ...
     • стоп-слова рекламы (erid как целое слово, реклам-, партнёрск-, промокод,
       скидк-) — пропуск; «erid» ищется словом, иначе ловится «meridian» в
       почте 38_meridian@inbox.ru (так и было в первом прогоне);
...
const СТОП_СЛОВА = [/(^|[^a-zа-яё0-9_])erid([^a-zа-яё0-9_]|$)/i, /реклам/i, /партн[её]рск/i, /промокод/i, /скидк/i];
const ПРИЗНАКИ_АНОНСА = [/записаться|записывайтесь|запись по ссылке|запись обязательна|регистрация по ссылке|зарегистрир/i, /\d\s*(₽|руб\.|руб\b|рублей|р\.)/i];
const ПРИВЕТСТВИЕ = /^\W*(дорог|здравств|добр(ый|ого|ое|ой)|привет|друзья|товарищ)/i;
...
function разобратьБлокТелеграма(блок, ист, о) {
  ...
  /* Здоровье: запись без текста, у которой при этом есть хоть что-то
     (фото, видео) или нет ничего вовсе, — признак пропавшего блока текста. */
  if (!полныйТекст && !bez_prevyu && !документ && !опрос) здоровье.bez_teksta++;
...
function разобратьТелеграм(html, ист, параметры) {
  ...
  const о = {
    своиКаналы,
    сейчасМс: п.сейчасМс,
    здоровье: { блоков: 0, служебных: 0, bez_daty: 0, bez_teksta: 0, photo_wrap: 0, bez_kartinki: 0 },
...
function здоровьеПлохое(з) {
  ...
  if (з.блоков >= МИН_ЗАПИСЕЙ_ДЛЯ_ПОРОГА) {
    ...
    if (з.bez_teksta / з.блоков > ПОРОГ_ЗДОРОВЬЯ) части.push('без текста ' + з.bez_teksta + ' из ' + з.блоков);
  }
