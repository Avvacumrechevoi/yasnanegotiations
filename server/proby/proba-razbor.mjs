/* Прогон разборщиков ленты (server/lenta-razbor.js) без сети: сохранённые
   страницы превью Телеграма и RSS Rutube из разведки 05.09.2026 плюс
   синтетические блоки на правила фильтров.

   Страницы в репозиторий не кладутся (чужие тексты и превью): путь к ним —
   переменная LENTA_STRANICY, по умолчанию каталог разведки в scratchpad.
...
if (!existsSync(join(СТРАНИЦЫ, 'russkaya_yasna.html'))) {
  console.error('нет сохранённых страниц в ' + СТРАНИЦЫ + ' — задай LENTA_STRANICY=<каталог с russkaya_yasna.html, …, rutube-rss.xml>');
  process.exit(2);
}
const страница = (имя) => readFileSync(join(СТРАНИЦЫ, имя), 'utf8');
...
/* ── 1. сохранённые страницы Телеграма ─────────────────────────────────── */
console.log('Сохранённые страницы Телеграма');
{
...
}
{
  ...
  так(р.отброшено.reklama === 4 && ид.join(',') === '14,15,16', 'стоп-слова erid/промокод/партнёрск/скидк → пропуск; meridian не в счёт', JSON.stringify([р.отброшено, ид]));
