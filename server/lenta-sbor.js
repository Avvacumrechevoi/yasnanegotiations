...
const СРОК_ЗАПРОСА_МС = 10000;
const СРОК_КАРТИНКИ_МС = 8000;
const МИН_ОСТАТОК_МС = 15000;              /* меньше — источники ждут следующего запуска */
...
async function роботРазрешает(адрес, путь, о) {
  ...
  if (!з || мс - з.когда > ROBOTS_КЭШ_МС) {
    let текст = '';
    try {
      const р = await о.взять('https://' + host + '/robots.txt', { срок: СРОК_ЗАПРОСА_МС });
...
async function собратьИсточник(ист, о) {
  ...
  let ответ;
  try { ответ = await о.взять(адрес, { срок: СРОК_ЗАПРОСА_МС }); }
  catch (e) { return итог(ист, о, начало, 'oshibka', 'сеть: ' + коротко(e && e.message || e), 0); }
