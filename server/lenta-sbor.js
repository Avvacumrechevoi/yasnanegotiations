/* ═══════════════════════════════════════════════════════════════════════════
   ...
   ОДИН ЗАПУСК:
     ...
     9. последним делом — СНИМОК первой страницы ленты в бакет
        (lenta/snimok.json, см. server/lenta-snimok.js). Домен шлюза
        06.09.2026 резолвится, но TCP 443 к нему из части сетей не
        открывается, а хранилище отвечает всем — через него же идёт
        обновление приложения. Снимок собирается ТЕМ ЖЕ кодом, что и ручка
        GET /lenta?n=20 (lenta.js собратьСтраницу), и кладётся, только если
        содержимое изменилось. Не положился — заход всё равно удачный, в
        журнал уходит строка о причине.
...
const Снимок = require('./lenta-snimok.js');
...
async function сбор(о, задание) {
  ...
  /* Снимок — последним делом: база уже переписана и убрана, значит в бакет
     ляжет то же, что отдала бы ручка прямо сейчас. Ни одна беда со снимком
     не отменяет захода: записи собраны и лежат в базе, а про снимок
     владельцу говорит строка журнала. */
  let снимок;
  const снятоС = Date.now();
  try { снимок = await Снимок.сделатьСнимок(о); }
  catch (e) { снимок = { polozhen: false, sboj: true, prichina: Снимок.коротко(e) }; }
  if (снимок.sboj) {
    console.error('[lenta-sbor] снимок ленты не положен: ' + снимок.prichina);
    try {
      await х.журнал({ klyuch: 'snimok', ishod: 'oshibka', novyh: 0, dlitelnost_ms: Math.max(0, Date.now() - снятоС),
        otkuda: о.otkuda, soobshchenie: коротко('снимок ленты не положен: ' + снимок.prichina, 300) });
    } catch (e) { console.error('[lenta-sbor] снимок: и строку журнала не записали', e); }
  }
  return { всего: все.length, опрошено: очередь.length, отчёт, уборка: убрано, snimok: снимок };
}
...
function бакетОблака(п) {
  ...
  return {
    ...
    /* кэш — четвёртым доводом: у картинок он недельный (копия отозванного
       фото не должна год жить в телефоне, но и меняться она не может), у
       снимка ленты — две минуты, он меняется каждые четверть часа. */
    async положить(ключ, тело, тип, кэш) {
      const р = await fetch(основа + ключ, {
        method: 'PUT',
        headers: Object.assign(заголовок(), { 'Content-Type': тип || 'image/jpeg', 'Cache-Control': кэш || CACHE_CONTROL_КАРТИНОК, 'Content-Length': String(тело.length) }),
        body: тело,
...
exports.окружениеОблака = async function окружениеОблака(context) {
  ...
  return {
    ...
    /* Первая страница ленты — тем же кодом, что и ручка GET /lenta. */
    страницаЛенты: Снимок.страницаЛентыYDB(drv, ydb),
    сейчас: () => Date.now(),
