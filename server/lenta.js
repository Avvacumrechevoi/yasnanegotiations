...
/* ─── сборка страницы: ОДИН код для ручки и для снимка ────────────────────── */
/* Отсюда берут страницу двое: ручка GET /lenta (ниже) и снимок первой
   страницы, который сборщик кладёт в бакет (server/lenta-snimok.js). Пока
   этот код был один на всю ручку, снимок пришлось бы писать вторым списком
   тех же правил — перемежение, skryto, Rutube только под видом «Видео»,
   sobrano_at по источникам ЭТОЙ выдачи, — и первое же расхождение клиент
   увидел бы как «в сети лента одна, без сети другая».

   Параметры сюда приходят УЖЕ проверенными: разбор и проверка запроса —
   дело ручки, а снимок никакого запроса не разбирает. Ответ не собирается
   (ни ok(), ни заголовки, ни журнал) — возвращается только тело и ветка,
   по которой оно получено:
     'pusto-n0'    — n=0 без posle: спрашивали только общее, записи не нужны;
     'bez-kanalov' — у управления нет ни одного канала;
     'stranica'    — обычная страница.
   Ветка нужна ручке, чтобы писать в журнал ровно те же строки, что и до
   выноса. */
async function собратьСтраницу(drv, д, п) {
  const upravlenie = п.upravlenie || null;
  const tip = п.tip || null;
  const n = п.n == null ? СТРАНИЦА : п.n;
  const курсор = п.курсор || null;
  const после = п.после || null;

  /* свежие: сборщик только что переписал lenta_istochniki и обязан читать их
     заново — пятиминутный кэш экземпляра отдал бы ему прежние zapisej и
     udacha_at, то есть снимок со вчерашней свежестью. Ручке кэш нужен. */
  const список = await источники(drv, д, !!п.свежие);
  ...
  const пусто = (novyh) => Object.assign({ zapisi: [], dalshe: null, novyh }, общее);
  if (n === 0 && !после) return { тело: пусто(null), ветка: 'pusto-n0' };
  if (каналы && !каналы.length) return { тело: пусто(после ? 0 : null), ветка: 'bez-kanalov' };
  ...
  return {
    тело: Object.assign({ zapisi: перемежить(записи).map(наружу), dalshe, novyh }, общее),
    ветка: 'stranica',
  };
}
...
async function лента(drv, ctx) {
  ...
  const { тело, ветка } = await собратьСтраницу(drv, д, { upravlenie, tip, n, курсор, после });

  if (ветка === 'pusto-n0') return ответ(ok, тело);
  if (ветка === 'bez-kanalov') {
    if (otkuda && пораВЖурнал()) {
      журналВПути = вЖурнал(drv, д, otkuda, `upravlenie=${upravlenie} bez-kanalov dolya=${ДОЛЯ_ЖУРНАЛА}`, 0, начало);
    }
    return ответ(ok, тело);
  }
...
/* Сборка страницы наружу — для снимка ленты в бакете (server/lenta-snimok.js).
   Один код на ручку и на снимок: см. комментарий у собратьСтраницу. */
exports.собратьСтраницу = собратьСтраницу;
exports.СТРАНИЦА = СТРАНИЦА;
