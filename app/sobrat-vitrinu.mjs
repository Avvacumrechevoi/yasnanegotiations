...
function поправить() {
  ...
  let замок = 0, работник = 0, прибавка = 0, объявлено = 0;
  ...
  return { страницы: страницы.length, замок, работник, прибавка, объявлено };
}
...
console.log(`  страниц: ${p.страницы} · снят замок: ${p.замок} · снят работник офлайна: ${p.работник} · добавлена прибавка: ${p.прибавка} · объявлено экранов: ${p.объявлено}`);
if (обломки.length) {
