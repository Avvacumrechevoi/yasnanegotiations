...
/* Пять вкладок по решению владельца: Сегодня · Уроки · Практика · Библиотека
   · Разбор. Профиль вкладкой быть перестал — он аватар в правом углу общей
   шапки (app/navigatsiya.js, аватарВШапке) и обычный дочерний экран: родителя
   ему не пишем нарочно, подсвечена будет вкладка, С КОТОРОЙ в него вошли (об
   этом знает стек оболочки, а не таблица). */
const ЭКРАНЫ = {
  'index.html':       { тип: 'корень', имя: 'Сегодня', имяИз: '.privet .im' },
  'learn.html':       { тип: 'корень', имя: 'Уроки' },
  'praktika.html':    { тип: 'корень', имя: 'Практика' },
  'biblioteka.html':  { тип: 'корень', имя: 'Библиотека' },
  ...
  'profil.html':      { тип: 'дочерний', имя: 'Профиль' },
  // Партия и Рейтинг живут внутри «Практики»: своей вкладки у них больше нет.
  'duel.html':        { тип: 'дочерний', имя: 'Игры', родитель: 'praktika.html' },
  'rating.html':      { тип: 'дочерний', имя: 'Рейтинг', родитель: 'praktika.html' },
  'negotiations.html':{ тип: 'дочерний', имя: 'Переговоры', родитель: 'praktika.html' },
  // Заголовок книги меняется на имя главы — ведём его за страницей.
  'kniga.html':       { тип: 'дочерний', имя: 'Книга', родитель: 'biblioteka.html',
                        имяИз: '#shapka-t' },
  // Двенадцать управлений центра — список под Библиотекой; вводные уроки
  // управлений открываются из него слоем lesson.html (?otkuda=upravleniya.html).
  'upravleniya.html': { тип: 'дочерний', имя: 'Управления', родитель: 'biblioteka.html' },
  // Лента записей из открытых каналов управлений — вторая дверь Библиотеки.
  // Лента живёт на «Сегодня» (раздел «Лента управлений»), в Библиотеке её
  // больше нет — значит и возврат с этого экрана ведёт на «Сегодня».
  'lenta.html':       { тип: 'дочерний', имя: 'Лента управлений', родитель: 'index.html' },
  // Урок — свой экран, а не слой поверх Разбора: он занимает экран целиком,
  // поэтому «слой» (шапка с ✕, наббара нет) и полотно без отступа сверху.
  'lesson.html':      { тип: 'слой', имя: 'Урок', родитель: 'learn.html', холст: true },
  // Круг открывают и из Практики, и из Уроков: вкладку оболочка подсветит по
  // метке ?otkuda, а родитель здесь — запасной путь для прямой ссылки.
  'games/krug/index.html': { тип: 'дочерний', имя: 'Разложи по кругу',
                             родитель: 'praktika.html', имяИз: '#krug-zagolovok' },
};
...
/* ═══ СТАРЫЙ АДРЕС УРОКА ВЕДЁТ НА НОВЫЙ ЭКРАН ════════════════════════════
   Урок жил слоем внутри Разбора, и открыть его значило загрузить весь
   конструктор: три касания, три полные перезагрузки и 3,8 МБ JS до первой
   отрисовки — при том что уроку не нужны ни объём, ни живая игра. Теперь
   урок — свой экран docs/lesson.html.

   Ссылок вида konstruktor.html?lesson=… в дереве знаний и на Главной больше
   сотни, и правят их другие руки. До той правки они обязаны работать —
   поэтому в приложении konstruktor.html с таким адресом сразу уходит на
   новый экран. replace, а не assign: своей записи в истории у перехода
   быть не должно, иначе «назад» из урока возвращала бы в него же.

   Почему правка живёт здесь, а не в docs/konstruktor.html: на сайте старый
   путь исправен и ничего не ломает, а витрина в приложении собирается этим
   файлом — здесь и место различию приложения от сайта. */
const ПЕРЕАДРЕСАЦИЯ_УРОКА = `\n<script>(function(){try{
  var к=new URLSearchParams(location.search), у=к.get('lesson');
  if(!у)return;
  var о=к.get('otkuda')||'';
  location.replace('lesson.html?lesson='+encodeURIComponent(у)+
    (о?'&otkuda='+encodeURIComponent(о):''));
}catch(e){}})();</script>`;
...
function поправить() {
  ...
  let замок = 0, работник = 0, прибавка = 0, объявлено = 0, токены = 0, урок = 0;
  for (const п of страницы) {
    ...
    // СТАРЫЙ АДРЕС УРОКА. Ставим сразу после <meta charset> — этот скрипт
    // обязан отработать РАНЬШЕ бандла Разбора, иначе конструктор успеет
    // смонтироваться и человек заплатит за него загрузкой.
    if (ключ === 'konstruktor.html' && !/lesson\.html\?lesson=/.test(s)) {
      const мЗ = s.match(/<meta charset[^>]*>/i);
      if (мЗ) {
        s = s.slice(0, мЗ.index + мЗ[0].length) + ПЕРЕАДРЕСАЦИЯ_УРОКА + s.slice(мЗ.index + мЗ[0].length);
        урок++;
      }
    }
  ...
  return { страницы: страницы.length, замок, работник, прибавка, объявлено, токены, урок };
}
...
console.log(`  страниц: ${p.страницы} · снят замок: ${p.замок} · снят работник офлайна: ${p.работник} · добавлена прибавка: ${p.прибавка} · объявлено экранов: ${p.объявлено} · дописаны роли: ${p.токены} · старый адрес урока переадресован: ${p.урок}`);
if (обломки.length) {
