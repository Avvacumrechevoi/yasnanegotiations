...
(function () {
  ...
  var РОДИЧ = { 'duel.html': 'praktika.html',
                ...
                /* Лента живёт на «Сегодня»: дверь в неё стоит там, а из
                   Библиотеки убрана. Пока родителем была Библиотека, «назад»
                   с ленты уводило на экран, где ленты уже нет. */
                'lenta.html': 'index.html' };
  ...
  var ПО_ОТКУДА = { uroki: 'learn.html', igra: 'praktika.html',
                    ...
                    /* Лента открывается дверью с «Сегодня». Метку lenta ставит
                       сама встроенная лента главной, когда с неё уходят на
                       полный экран («Только «X»»), — и возвращать надо туда
                       же, на «Сегодня», а не в Библиотеку. */
                    segodnya: 'index.html', lenta: 'index.html',
  ...
  /* Экран-полотно, которое возит содержимое вбок по делу, снимает с себя
     страховку по ширине (см. правило html.yk-est:not(.yk-shire) ниже). */
  if (тело && тело.hasAttribute('data-yk-shire')) КОРЕНЬ_HTML.classList.add('yk-shire');
  ...
  st.textContent =
    ...
    /* Клавиатура закрывает нижнюю треть экрана — полосе там не место.
       Правило одно, а решает, ставить ли класс, блок 7: цена ошибки высока —
       без наббара с экрана нечем уйти. */
    '.yk-nav.yk-klava{display:none}' +

    /* ── ШИРИНА ЭКРАНА — ПРЕДЕЛ ДЛЯ ВСЕГО ─────────────────────────────────
       Жалоба владельца «страница двигается вбок, как вебвью» — это не одна
       вёрстка одного экрана, а класс ошибки: любая карточка с отрицательным
       полем, длинное слово, широкая таблица или лента без своей прокрутки
       растягивают ДОКУМЕНТ, и приложение начинает ездить вбок целиком —
       вместе с шапкой и содержимым. Родное приложение так не делает никогда.

       Ставим предел здесь, в оболочке, а не на каждом экране: clip, а не
       hidden. Разница существенная, а не вкусовая.
         • overflow-x:hidden делает область ПРОКРУЧИВАЕМОЙ (просто без полос):
           появляется свой скроллпорт, и position:sticky внутри начинает
           считаться от него; сочетание hidden по одной оси с visible по
           другой невозможно — visible молча превращается в auto, и документ
           получает второй скролл-контейнер (отсюда классические «две
           прокрутки» и прыгающий sticky).
         • overflow-x:clip скроллпорта НЕ создаёт: лишнее просто отрезано,
           overflow-y остаётся visible (пара clip+visible спекой разрешена),
           прокрутка страницы, position:fixed наббара и любые sticky-ленты
           работают ровно как раньше.
       Намеренные горизонтальные ленты (карусель управлений на «Сегодня»,
       чипы Ленты) не страдают: у них СВОЙ overflow-x:auto, то есть свой
       скроллпорт внутри экрана, и обрезка документа их не касается.
       Экран-полотно, которому горизонтальная прокрутка нужна по делу,
       отказывается от страховки атрибутом data-yk-shire на body. */
    '@supports (overflow-x:clip){html.yk-est:not(.yk-shire){overflow-x:clip}}' +
  ...
  /* ══ 7. КЛАВИАТУРА ═════════════════════════════════════════════════════
     Наббар прячется ровно в одном случае: под полем ввода всплыла настоящая
     клавиатура. Всё остальное время он стоит — с него уходят с экрана, и
     исчезнувшая полоса читается как «приложение сломалось».

     Что было и почему пропадало. Признаком считали одну голую разницу:
     «высота html минус (visualViewport.height + offsetTop) больше 140 —
     значит клавиатура». Разница эта растёт не только от клавиатуры:
       • пинч-зум и двойной тап: visual viewport при увеличении в полтора
         раза становится на треть ниже layout — на 900 px окна это 300 px
         «инсета» из ниоткуда, и наббар исчезал от простого приближения;
       • прокрутка в увеличенном виде: offsetTop идёт вверх-вниз, разница
         скачет, класс мигает на каждый кадр прокрутки;
       • системные панели и оттяжка (overscroll) в WebView: несколько
         десятков-сотен пикселей на границе, порог один — значит дребезг
         вокруг него, класс то ставится, то снимается;
       • панель поиска/подсказок и выпадающие системные листы: viewport
         ужимается, поля ввода при этом нет вовсе.
     Ни в одном из этих случаев клавиатуры нет, а наббар пропадал.

     Стало — три правила, по убыванию силы:
       1) родное событие моста yasna:клавиатура (app/pribavka.js слушает
          плагин Capacitor) — правда от системы, эвристика при нём молчит;
       2) НЕТ СФОКУСИРОВАННОГО ПОЛЯ — НЕТ КЛАВИАТУРЫ. Без каретки система
          её не поднимает; на экранах вовсе без полей («Сегодня», «Уроки»,
          «Практика», «Библиотека») наббар теперь не скрывается никогда, что
          бы ни делал viewport;
       3) запасная эвристика с гистерезисом: чтобы полосу СПРЯТАТЬ, нужно
          съесть 180 px снизу, чтобы ВЕРНУТЬ — опуститься ниже 100. Разные
          пороги убирают дребезг на границе; при зуме (scale ≠ 1) эвристика
          не считает вовсе — там разница говорит о масштабе, а не об IME.

     ВАЖНО ПРО СБОРКУ: плагина @capacitor/keyboard в app/package.json нет,
     значит ветка (1) сейчас молчит и всё держится на (2)+(3). Поэтому
     запасная эвристика написана как основная, а не как «на всякий случай». */
  var ПОРОГ_СКРЫТЬ = 180;      /* px снизу, чтобы поверить в клавиатуру */
  var ПОРОГ_ВЕРНУТЬ = 100;     /* px, ниже которых считаем, что её убрали */
  var сНатива = null;          /* что сказал мост: null — не говорил ничего */
  var клаваВидна = false;      /* текущее решение (нужно для гистерезиса) */
  var vv = window.visualViewport;

  /* Поля, которые клавиатуру не поднимают: галочки, переключатели, кнопки,
     выбор файла и цвета — системные окна, а не ввод. */
  var БЕЗ_КЛАВИШ = { checkbox: 1, radio: 1, button: 1, submit: 1, reset: 1,
                     file: 1, image: 1, range: 1, color: 1, hidden: 1 };

  function полеВФокусе() {
    var э = document.activeElement;
    /* Фокус может стоять в теневом дереве (карточка входа, шторка): у хоста
       activeElement — сам хост, спускаемся внутрь. */
    for (var шаг = 0; шаг < 6 && э && э.shadowRoot && э.shadowRoot.activeElement; шаг++) {
      э = э.shadowRoot.activeElement;
    }
    if (!э || э === document.body || э === КОРЕНЬ_HTML) return false;
    var имя = (э.tagName || '').toLowerCase();
    if (имя === 'textarea') return true;
    if (имя === 'select') return false;            /* системный список, не ввод */
    /* Внутрь чужого документа не заглянуть — считаем, что там может быть
       поле: лучше спрятать полосу зря, чем оставить её поверх клавиатуры. */
    if (имя === 'iframe') return true;
    if (имя === 'input') {
      var тип = (э.getAttribute('type') || 'text').toLowerCase();
      return !БЕЗ_КЛАВИШ[тип];
    }
    return э.isContentEditable === true;
  }

  /* Сколько нижней части окна отъедено. Считаем в пикселях разметки: и
     clientHeight, и offsetTop живут в них, а vv.height при зуме — нет,
     поэтому при scale ≠ 1 значение не используется (см. решить()). */
  function отъедено() {
    if (!vv) return 0;
    return КОРЕНЬ_HTML.clientHeight - (vv.height + vv.offsetTop);
  }

  function решить() {
    var поле = полеВФокусе();
    /* Мост знает правду от системы, но и он не отменяет правила «нет поля —
       нет клавиатуры»: если событие «показалась» осталось без пары (экран
       сменился, поле сняло фокус), полоса вернётся сама. */
    if (сНатива !== null) return сНатива && поле;
    if (!поле) return false;
    if (!vv) return false;
    var масштаб = vv.scale || 1;
    if (масштаб > 1.02 || масштаб < 0.98) return клаваВидна;  /* зум — не судья */
    var зазор = отъедено();
    return клаваВидна ? (зазор > ПОРОГ_ВЕРНУТЬ) : (зазор > ПОРОГ_СКРЫТЬ);
  }

  function проверитьКлавиатуру() {
    var видна = !!решить();
    if (видна === клаваВидна) return;
    клаваВидна = видна;
  ...
  /* Фокус — второй источник наравне с размером окна: клавиатура приходит и
     уходит вслед за ним. focusout ждёт кадр: фокус часто перескакивает с
     поля на поле, и мигать полосой между ними незачем. */
  document.addEventListener('focusin', проверитьКлавиатуру, true);
  document.addEventListener('focusout', function () {
    setTimeout(проверитьКлавиатуру, 0);
  }, true);
  ...
  /* Наружу — для проб и отладки: чем именно объяснено текущее состояние. */
  yasnaNav.клавиатура = function () {
    return { видна: клаваВидна, поле: полеВФокусе(), сНатива: сНатива,
             зазор: Math.round(отъедено()), масштаб: vv ? (vv.scale || 1) : 1 };
  };
