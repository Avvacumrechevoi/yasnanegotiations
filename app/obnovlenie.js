...
(function () {
  ...
  /* Куда идти, когда обновиться из приложения нельзя. Домен yasnalab.ru ещё
     не делегирован, поэтому адрес хранилищный — он ходит из России. */
  var СТРАНИЦА = 'storage.yandexcloud.net/yasnalab.ru/app.html';
  var АДРЕС_СТРАНИЦЫ_СЛОВАМИ =
    'Обновиться отсюда не выйдет: эта версия выложена без отпечатка файла, '
    + 'и сверить скачанное нечем. Откройте в браузере ' + СТРАНИЦА
    + ' — там та же версия, ставится поверх, данные сохранятся.';
  ...
  window.yasnaObnovaSkachat = function (url, ход) {
    ...
    if (!прочитано.sha256) {
      /* Тупик: обновиться отсюда нельзя. Значит надо не просто отказать, а
         сказать КУДА идти — иначе человек остаётся с кнопкой, которая молча
         не работает. Так и случилось на сборках 6.4–7.3: там размер терялся
         на мосту, и эта ветка срабатывала на исправном манифесте. */
      скажи({ вид: 'ошибка', текст: АДРЕС_СТРАНИЦЫ_СЛОВАМИ });
      return Promise.resolve();
    }
