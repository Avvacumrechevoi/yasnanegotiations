...
extras_for(){
  case "$1" in
    ...
    spar)          echo "access.js" ;;
    # lenta.js едет к сборщику НЕ ради ручек: из него берётся собратьСтраницу —
    # тот самый код, которым отвечает GET /lenta. Снимок первой страницы в
    # бакете обязан быть той же формы, что ответ ручки, иначе клиенту пришлось
    # бы разбирать их двумя путями. Своих зависимостей у lenta.js нет (crypto
    # и внедряемые ctx.д), так что пакет сборщика от него не тяжелеет.
    lenta-sbor)    echo "lenta-razbor.js jpeg-js.js lenta-snimok.js lenta.js" ;;
    *)             echo "" ;;
  esac
}
