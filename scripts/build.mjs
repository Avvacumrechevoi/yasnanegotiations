...
import { собратьИсходники, проверитьТексты, проверитьДерево, напечатать } from './proverki.mjs';
...
async function buildAll(){
  ...
  for(const t of targets){
    ...
    /* ─── Проверки текста и дерева ──────────────────────────────────────
       Правила продукта («в приложении пока нет» вместо «скоро», продукт
       говорит от себя, место круга зовётся местом) держались только на
       памяти — и один заход правок ушёл на то, чтобы вычистить их заново.
       Теперь их проверяет сборка: правила в scripts/proverki.mjs, там же
       написано, что считается видимым текстом. Дерево знаний смотрим
       отдельно — дубль id в пятистах строках глазами не находится, а
       именно он заставлял книгу открывать экран Игр.
       Собираем бандлы ДО проверки нарочно: человеку, который правит текст,
       нужен и свежий dist, и список мест — а не выбор между ними. */
    const находки = проверитьТексты(собратьИсходники(srcRoot))
      .concat(проверитьДерево(path.join(srcRoot, 'core', 'derevo-dannye.js')));
    if (напечатать(находки, t.src + '/')) process.exitCode = 1;
  }

  /* Галочку ставим только когда проверки прошли: «✅ Готово» над списком
     нарушений — та самая ложь интерфейса, только в консоли. */
  console.log(`\n${process.exitCode ? '⚠ Собрано, но проверки не пройдены' : '✅ Готово'} за ${Date.now() - start} мс`);
}
