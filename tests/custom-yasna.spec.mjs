...
// Замок (docs/core/gate.js) уводит на главную с любой страницы кроме неё
// самой. Тестам нужен пройденный замок, поэтому ставим признак ДО загрузки —
// addInitScript выполняется раньше скриптов документа, то есть раньше замка.
